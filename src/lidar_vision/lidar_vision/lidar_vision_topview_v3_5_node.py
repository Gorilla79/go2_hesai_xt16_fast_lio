...
class LiDARVisionTopViewV35Node(Node):
    ...
    def compute_360_sector_state(self, safety_points):
        ...
        for name, sector_mask in masks.items():
            sector_dist = distances[sector_mask]
            result = self.evaluate_sector_distances(sector_dist)
            state["sectors"][name] = result
            risks.append(result["risk"])

        state["overall_risk"] = self.max_risk(risks)
        state["reason"] = "sector_analysis_complete"
        return state

    def evaluate_sector_distances(self, distances):
        result = self.empty_sector_result()
        count = int(len(distances))
        result["point_count"] = count

        if count < self.sector_min_points:
            result["risk"] = "CLEAR"
            return result

        percentile = max(0.0, min(50.0, self.sector_distance_percentile))
        nearest = float(np.percentile(distances, percentile))
        result["nearest_distance"] = nearest
        result["occupied"] = True

        if nearest <= self.sector_stop_distance:
            result["risk"] = "STOP"
        elif nearest <= self.sector_slow_distance:
            result["risk"] = "SLOW"
        elif nearest <= self.sector_caution_distance:
            result["risk"] = "CAUTION"
        else:
            result["risk"] = "CLEAR"
    ...
    def compute_lidar_cluster_candidates(self, obstacle_state, source_points=None):
        ...
        if len(pts) > self.obstacle_draw_max_points:
            idx = np.random.choice(len(pts), self.obstacle_draw_max_points, replace=False)
            pts = pts[idx]
    ...
    def draw_360_sector_status(self, canvas, sector_state):
        ...
        for name in ["FRONT", "LEFT", "RIGHT", "REAR"]:
            ...
            text = f"{name}: {risk} | d={nearest_text} | n={count}"
