...
class LiDARVisionTopViewV36Node(Node):
    """
    ...
    def __init__(self):
        super().__init__("lidar_vision_topview_v3_6_node")
        ...
        # V3.6 LiDAR cluster temporal tracks
        ...
        self.get_logger().info("LiDAR_Vision top-view V3.6 node started.")
        ...
        self.get_logger().info(
            f"V3.6 tracking enable={self.enable_tracking}, "
        ...
        self.get_logger().info(
            f"V3.6 duplicate filter enable={self.enable_duplicate_filter}, "
        ...
        self.get_logger().info(
            f"V3.6 false-positive filter enable={self.enable_false_positive_filter}, "
        ...
        self.get_logger().info(
            f"V3.6 LiDAR obstacle layer enable={self.enable_lidar_obstacle_layer}, "
        ...
        self.get_logger().info(
            f"V3.6 LiDAR cluster layer enable={self.enable_lidar_cluster_layer}, "
        ...
        self.get_logger().info(
            f"V3.6 360 BEV enable={self.enable_360_bev}, "
        ...
        self.get_logger().info(
            f"V3.6 cluster tracking enable={self.enable_lidar_cluster_tracking}, "
    ...
    # ============================================================
    # V3.6 LiDAR cluster temporal tracking
    # ============================================================
    @staticmethod
    ...
    def lidar_cluster_match_cost(self, track, cluster):
        ...
        position_distance = float(
            np.linalg.norm(track_center[:2] - cluster_center[:2])
        )

        if position_distance > self.cluster_track_match_distance:
            return None
        ...
        return (
            position_distance
            + self.cluster_track_size_weight * size_difference
        )
    ...
    def update_lidar_cluster_track_from_detection(
        self,
        track,
        cluster,
        timestamp,
        ...
        alpha = max(
            0.01,
            min(1.0, self.cluster_track_smoothing_alpha),
        )

        old_center = np.asarray(track["center"], dtype=np.float32)
        new_center = self.cluster_center_array(cluster)
        smoothed_center = alpha * new_center + (1.0 - alpha) * old_center
        ...
        track["center"] = tuple(float(v) for v in smoothed_center)
        track["min_xyz"] = tuple(float(v) for v in smoothed_min)
        track["max_xyz"] = tuple(float(v) for v in smoothed_max)

        track["width"] = float(
            alpha * cluster["width"] + (1.0 - alpha) * track["width"]
        )
        track["depth"] = float(
            alpha * cluster["depth"] + (1.0 - alpha) * track["depth"]
        )
        track["height"] = float(
            alpha * cluster["height"] + (1.0 - alpha) * track["height"]
        )
        ...
        track["required_point_count"] = int(
            cluster.get("required_point_count", 0)
        )
        track["candidate_type"] = cluster.get(
            "candidate_type",
            "UNKNOWN_OBSTACLE",
        )
        track["control_relevant"] = bool(
            cluster.get("control_relevant", False)
        )
    ...
    def update_lidar_cluster_tracks(self, clusters, timestamp):
        """
        Stabilize 360-degree LiDAR clusters over time.

        This version does not classify objects as moving or static.
        It only maintains identity, persistence, center and size.
        """
        ...
        for track_index, track in enumerate(self.lidar_cluster_tracks):
            for cluster_index, cluster in enumerate(clusters):
                ...
                candidate_pairs.append(
                    (cost, track_index, cluster_index)
                )
        ...
        for _, track_index, cluster_index in candidate_pairs:
            if (
        ...
        for cluster_index, cluster in enumerate(clusters):
            ...
            self.lidar_cluster_tracks.append(
                self.create_lidar_cluster_track(cluster, timestamp)
            )
    ...
    def draw_yolo_view(self, frame, persons):
        ...
        status = (
            f"LiDAR_Vision V3.6 | tracks={len(persons)} | control={control_count} | avg={avg_ms:.1f}ms | "
    ...
    def draw_lidar_clusters_topview(self, canvas, clusters, origin_x, origin_y):
        for cluster in clusters:
            ...
            label = (
                ...
                f"{cluster.get('track_state', 'TENTATIVE')} "
                f"{cluster.get('sector', '?')} {ctype} "
                f"{cluster.get('distance', 0.0):.1f}m "
                f"hit={cluster.get('hit_count', 0)} "
                f"miss={cluster.get('missed_frames', 0)}"
            )
    ...
    def draw_cluster_status(self, canvas, clusters):
        ...
        text = (
            f"LiDAR Tracks | total={counts['TOTAL']} | "
            f"stable={stable_tracks} | stale={stale_tracks} | "
            f"human_support={counts.get('CONFIRMED_HUMAN_SUPPORT', 0)} | "
...
def main(args=None):
    ...
    try:
        node = LiDARVisionTopViewV36Node()
        rclpy.spin(node)
