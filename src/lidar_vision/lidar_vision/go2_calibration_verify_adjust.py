...
class Go2CalibrationVerifyAdjust(Node):
    """
    GO2 RealSense D435i + Hesai XT16 calibration verification and adjustment tool.

    Purpose:
        1. Load previously successful calibration values as default.
        2. Visualize RealSense depth cloud and Hesai LiDAR cloud in 3D.
        3. Check whether the camera position changed.
        4. Adjust camera / LiDAR extrinsics with OpenCV trackbars if needed.
        5. Print and save updated calibration values.
    ...
    Visualization:
        Green      : RealSense depth cloud
        Red/Orange : Hesai XT16 LiDAR cloud
        Robot frame: base origin
    """

    def __init__(self, args):
        super().__init__("go2_calibration_verify_adjust")
        ...
        # ------------------------------------------------------------
        # Previously successful calibration values
        # ------------------------------------------------------------
        ...
        print("[INFO] Calibration verification tool started.", flush=True)
        print("[INFO] Previously successful calibration values are loaded as default.", flush=True)
        print("[INFO] RealSense depth cloud: Green.", flush=True)
        print("[INFO] Hesai LiDAR cloud: Red/Orange.", flush=True)
        print("[INFO] If clouds overlap well, calibration is still valid.", flush=True)
        print("[INFO] Keys: s=print/save params, r=reset, q/ESC=quit.", flush=True)
    ...
    def create_control_window(self):
        cv2.namedWindow("GO2 Calibration Verify Controls", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("GO2 Calibration Verify Controls", 820, 700)
        cv2.moveWindow("GO2 Calibration Verify Controls", 30, 40)

        blank = np.zeros((700, 820, 3), dtype=np.uint8)
        cv2.imshow("GO2 Calibration Verify Controls", blank)

        # Camera rotation: -45.0 ~ +45.0 deg
        cv2.createTrackbar(
            "cam_roll_x10+450",
            "GO2 Calibration Verify Controls",
        ...
        # Camera position offset from successful calibration: -30cm ~ +30cm
        cv2.createTrackbar("cam_dx_cm+30", "GO2 Calibration Verify Controls", 30, 60, self.on_trackbar)
        cv2.createTrackbar("cam_dy_cm+30", "GO2 Calibration Verify Controls", 30, 60, self.on_trackbar)
        cv2.createTrackbar("cam_dz_cm+30", "GO2 Calibration Verify Controls", 30, 60, self.on_trackbar)
        ...
        # LiDAR position offset from successful calibration: -30cm ~ +30cm
        cv2.createTrackbar("lidar_dx_cm+30", "GO2 Calibration Verify Controls", 30, 60, self.on_trackbar)
        cv2.createTrackbar("lidar_dy_cm+30", "GO2 Calibration Verify Controls", 30, 60, self.on_trackbar)
        cv2.createTrackbar("lidar_dz_cm+30", "GO2 Calibration Verify Controls", 30, 60, self.on_trackbar)
    ...
    def create_open3d_window(self):
        ...
        self.vis.create_window(
            window_name="GO2 Calibration Verify - RealSense Depth Cloud vs Hesai LiDAR Cloud",
            width=1280,
            height=800,
            left=880,
            top=60,
        ...
        self.pcd_rs.colors = o3d.utility.Vector3dVector(np.array([[0.0, 0.85, 0.35]]))
    ...
    def read_trackbars(self):
        try:
            self.camera_roll_deg = (
                cv2.getTrackbarPos("cam_roll_x10+450", "GO2 Calibration Verify Controls") - 450
            ) / 10.0

            self.camera_pitch_deg = (
                cv2.getTrackbarPos("cam_pitch_x10+450", "GO2 Calibration Verify Controls") - 450
            ) / 10.0

            self.camera_yaw_deg = (
                cv2.getTrackbarPos("cam_yaw_x10+450", "GO2 Calibration Verify Controls") - 450
            ) / 10.0

            cam_dx = (
                cv2.getTrackbarPos("cam_dx_cm+30", "GO2 Calibration Verify Controls") - 30
            ) / 100.0
            cam_dy = (
                cv2.getTrackbarPos("cam_dy_cm+30", "GO2 Calibration Verify Controls") - 30
            ) / 100.0
            cam_dz = (
                cv2.getTrackbarPos("cam_dz_cm+30", "GO2 Calibration Verify Controls") - 30
            ) / 100.0
            ...
            self.lidar_roll_deg = (
                cv2.getTrackbarPos("lidar_roll_x10+1800", "GO2 Calibration Verify Controls") - 1800
            ) / 10.0

            self.lidar_pitch_deg = (
                cv2.getTrackbarPos("lidar_pitch_x10+1800", "GO2 Calibration Verify Controls") - 1800
            ) / 10.0

            self.lidar_yaw_deg = (
                cv2.getTrackbarPos("lidar_yaw_x10+1800", "GO2 Calibration Verify Controls") - 1800
            ) / 10.0

            lidar_dx = (
                cv2.getTrackbarPos("lidar_dx_cm+30", "GO2 Calibration Verify Controls") - 30
            ) / 100.0
            lidar_dy = (
                cv2.getTrackbarPos("lidar_dy_cm+30", "GO2 Calibration Verify Controls") - 30
            ) / 100.0
            lidar_dz = (
                cv2.getTrackbarPos("lidar_dz_cm+30", "GO2 Calibration Verify Controls") - 30
            ) / 100.0
    ...
    def reset_trackbars_to_default(self):
        print("[INFO] Resetting to previously successful calibration values.", flush=True)
        ...
        try:
            ...
            cv2.setTrackbarPos("cam_dx_cm+30", "GO2 Calibration Verify Controls", 30)
            cv2.setTrackbarPos("cam_dy_cm+30", "GO2 Calibration Verify Controls", 30)
            cv2.setTrackbarPos("cam_dz_cm+30", "GO2 Calibration Verify Controls", 30)
            ...
            cv2.setTrackbarPos("lidar_dx_cm+30", "GO2 Calibration Verify Controls", 30)
            cv2.setTrackbarPos("lidar_dy_cm+30", "GO2 Calibration Verify Controls", 30)
            cv2.setTrackbarPos("lidar_dz_cm+30", "GO2 Calibration Verify Controls", 30)
    ...
    def update_control_panel(self, rs_points_base, lidar_points_base):
        panel = np.zeros((700, 820, 3), dtype=np.uint8)
        panel[:] = (28, 28, 28)
        ...
        lines = [
            "GO2 Calibration Verify / Adjust",
            "",
            "Goal:",
            "Check whether the RealSense camera position changed.",
            "If green and red/orange clouds overlap well, calibration is valid.",
            "",
            "Colors:",
            "Green      : RealSense D435i depth cloud",
            "Red/Orange : Hesai XT16 LiDAR cloud",
            "",
            "Loaded successful defaults:",
            f"Cam default xyz   : ({self.default_camera_base_x:.3f}, {self.default_camera_base_y:.3f}, {self.default_camera_base_z:.3f}) m",
            f"Cam default rpy   : ({self.default_camera_roll_deg:.1f}, {self.default_camera_pitch_deg:.1f}, {self.default_camera_yaw_deg:.1f}) deg",
            f"LiDAR default xyz : ({self.default_lidar_base_x:.3f}, {self.default_lidar_base_y:.3f}, {self.default_lidar_base_z:.3f}) m",
            f"LiDAR default rpy : ({self.default_lidar_roll_deg:.1f}, {self.default_lidar_pitch_deg:.1f}, {self.default_lidar_yaw_deg:.1f}) deg",
            "",
            "Current values:",
            ...
            "",
            "Keys:",
            "s: print and save current parameters",
            "r: reset to previously successful calibration",
            "q/ESC: quit",
        ]
        ...
        cv2.imshow("GO2 Calibration Verify Controls", panel)
    ...
    def get_current_params_dict(self):
        return {
            "camera_base_x": round(float(self.camera_base_x), 4),
            "camera_base_y": round(float(self.camera_base_y), 4),
            "camera_base_z": round(float(self.camera_base_z), 4),
            "camera_roll_deg": round(float(self.camera_roll_deg), 3),
            "camera_pitch_deg": round(float(self.camera_pitch_deg), 3),
            "camera_yaw_deg": round(float(self.camera_yaw_deg), 3),
            "lidar_base_x": round(float(self.lidar_base_x), 4),
            "lidar_base_y": round(float(self.lidar_base_y), 4),
            "lidar_base_z": round(float(self.lidar_base_z), 4),
            "lidar_roll_deg": round(float(self.lidar_roll_deg), 3),
            "lidar_pitch_deg": round(float(self.lidar_pitch_deg), 3),
            "lidar_yaw_deg": round(float(self.lidar_yaw_deg), 3),
        }
    ...
    def print_current_params(self):
        ...
        print("\n---------- launch.py parameter style ----------", flush=True)
        print(f'"camera_base_x": {params["camera_base_x"]},', flush=True)
        print(f'"camera_base_y": {params["camera_base_y"]},', flush=True)
        print(f'"camera_base_z": {params["camera_base_z"]},', flush=True)
        print(f'"camera_roll_deg": {params["camera_roll_deg"]},', flush=True)
        print(f'"camera_pitch_deg": {params["camera_pitch_deg"]},', flush=True)
        print(f'"camera_yaw_deg": {params["camera_yaw_deg"]},', flush=True)
        print(f'"lidar_base_x": {params["lidar_base_x"]},', flush=True)
        print(f'"lidar_base_y": {params["lidar_base_y"]},', flush=True)
        print(f'"lidar_base_z": {params["lidar_base_z"]},', flush=True)
        print(f'"lidar_roll_deg": {params["lidar_roll_deg"]},', flush=True)
        print(f'"lidar_pitch_deg": {params["lidar_pitch_deg"]},', flush=True)
        print(f'"lidar_yaw_deg": {params["lidar_yaw_deg"]},', flush=True)

        print("\n---------- command-line style ----------", flush=True)
        print(f"--camera-base-x {params['camera_base_x']} \\")
        print(f"--camera-base-y {params['camera_base_y']} \\")
        print(f"--camera-base-z {params['camera_base_z']} \\")
        print(f"--camera-roll-deg {params['camera_roll_deg']} \\")
        print(f"--camera-pitch-deg {params['camera_pitch_deg']} \\")
        print(f"--camera-yaw-deg {params['camera_yaw_deg']} \\")
        print(f"--lidar-base-x {params['lidar_base_x']} \\")
        print(f"--lidar-base-y {params['lidar_base_y']} \\")
        print(f"--lidar-base-z {params['lidar_base_z']} \\")
        print(f"--lidar-roll-deg {params['lidar_roll_deg']} \\")
        print(f"--lidar-pitch-deg {params['lidar_pitch_deg']} \\")
        print(f"--lidar-yaw-deg {params['lidar_yaw_deg']}")
        print("====================================================\n", flush=True)

    def save_current_params(self):
        params = self.get_current_params_dict()
        save_path = os.path.expanduser(self.args.save_path)

        try:
            with open(save_path, "w", encoding="utf-8") as f:
                json.dump(params, f, indent=2)

            print(f"[INFO] Calibration parameters saved to: {save_path}", flush=True)

        except Exception as e:
            print(f"[WARN] Failed to save calibration parameters: {e}", flush=True)
...
def parse_args():
    ...
    # Previously successful calibration values
    parser.add_argument("--camera-base-x", type=float, default=0.340)
    parser.add_argument("--camera-base-y", type=float, default=0.040)
    parser.add_argument("--camera-base-z", type=float, default=0.390)
    parser.add_argument("--camera-roll-deg", type=float, default=0.0)
    parser.add_argument("--camera-pitch-deg", type=float, default=0.0)
    parser.add_argument("--camera-yaw-deg", type=float, default=-0.5)

    parser.add_argument("--lidar-base-x", type=float, default=0.150)
    parser.add_argument("--lidar-base-y", type=float, default=0.000)
    parser.add_argument("--lidar-base-z", type=float, default=0.250)
    parser.add_argument("--lidar-roll-deg", type=float, default=-6.0)
    ...
    parser.add_argument(
        "--save-path",
        default="~/go2_ws/latest_go2_calibration.json",
    )
...
def main():
    ...
    try:
        app = Go2CalibrationVerifyAdjust(args)
        app.run()
