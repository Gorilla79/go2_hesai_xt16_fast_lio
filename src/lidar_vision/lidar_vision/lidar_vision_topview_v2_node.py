...
class LiDARVisionTopViewNode(Node):
    """
    ...
    def __init__(self):
        super().__init__("lidar_vision_topview_node")
        ...
        self.get_logger().info("LiDAR_Vision top-view node started.")
    ...
    def draw_yolo_view(self, frame, persons):
        ...
        for person in persons:
            ...
            label = (
                f"P{person['id']} {source} "
                f"{person['distance']:.2f}m "
                f"x={x:.2f} y={y:.2f} n={person['lidar_count']}"
            )
        ...
        status = (
            f"LiDAR_Vision | persons={len(persons)} | avg={avg_ms:.1f}ms | "
            f"image LiDAR overlay=OFF | Green=LiDAR, Red=fallback"
        )
    ...
    def draw_topview_labels(self, canvas, origin_x, origin_y):
        ...
        cv2.putText(canvas,
                    "Gray: downsampled LiDAR | Green: LiDAR-associated person | Red: fallback",
    ...
    def print_status(self, lidar_points, persons):
        ...
        for p in persons:
            ...
            self.get_logger().info(
                f"  P{p['id']} {p['source']} dist={p['distance']:.2f}m "
                f"x={x:.2f}, y={y:.2f}, z={z:.2f}, "
                f"conf={p['confidence']:.2f}, n={p['lidar_count']}"
            )
