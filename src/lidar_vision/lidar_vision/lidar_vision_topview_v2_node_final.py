...
class LiDARVisionTopViewV2Node(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("LiDAR_Vision top-view V2 node started.")
    ...
    def main_loop(self):
        ...
        persons, infer_ms = self.run_yolo_lidar(frame, lidar_points_base)
    ...
    def draw_yolo_view(self, frame, persons):
        ...
        for person in persons:
            ...
            color = self.get_confidence_color(position_confidence)
            ...
            label = (
                f"P{person['id']} {source} {position_confidence} "
                f"{person['distance']:.2f}m "
                f"x={x:.2f} y={y:.2f} "
                f"n={person['lidar_count']} "
                f"std={person.get('depth_std', 999.0):.2f}"
            )
        ...
        status = (
            f"LiDAR_Vision V2 | persons={len(persons)} | avg={avg_ms:.1f}ms | "
            f"image LiDAR overlay=OFF | Green=HIGH, Yellow=MED, Orange=LOW, Red=FALLBACK"
        )
    ...
    def draw_person_on_topview(self, canvas, person, origin_x, origin_y):
        ...
        text1 = f"P{person['id']} {dist:.2f}m {position_confidence}"
        text2 = (
            f"x={x:.2f}, y={y:.2f}, "
            f"n={person['lidar_count']} "
            f"std={person.get('depth_std', 999.0):.2f}"
    ...
    def draw_topview_labels(self, canvas, origin_x, origin_y):
        ...
        cv2.putText(canvas,
                    "Gray: LiDAR | Green: HIGH | Yellow: MEDIUM | Orange: LOW | Red: FALLBACK",
    ...
    def print_status(self, lidar_points, persons):
        ...
        self.get_logger().info(
            f"status | persons={len(persons)} | avg={avg_ms:.1f}ms | "
            f"lidar_process_points={lidar_n}"
        )
        ...
        for p in persons:
            ...
            self.get_logger().info(
                f"  P{p['id']} {p['source']} "
                f"{p.get('position_confidence', 'FALLBACK')} "
