...
class LiDARVisionTopViewV34Node(Node):
    """
    ...
    def __init__(self):
        super().__init__("lidar_vision_topview_v3_4_node")
        ...
        self.get_logger().info("LiDAR_Vision top-view V3.4 node started.")
        ...
        self.get_logger().info(
            f"V3.4 tracking enable={self.enable_tracking}, "
        ...
        self.get_logger().info(
            f"V3.4 duplicate filter enable={self.enable_duplicate_filter}, "
        ...
        self.get_logger().info(
            f"V3.4 false-positive filter enable={self.enable_false_positive_filter}, "
        ...
        self.get_logger().info(
            f"V3.4 LiDAR obstacle layer enable={self.enable_lidar_obstacle_layer}, "
        ...
        self.get_logger().info(
            f"V3.4 LiDAR cluster layer enable={self.enable_lidar_cluster_layer}, "
    ...
    def main_loop(self):
        ...
        obstacle_state = self.compute_lidar_obstacle_layer(lidar_points_base)
        self.latest_obstacle_state = obstacle_state

        lidar_clusters = self.compute_lidar_cluster_candidates(obstacle_state)
        ...
        if self.draw_topview_window:
            top = self.draw_top_view(persons, lidar_points_topview, obstacle_state, lidar_clusters)
            cv2.imshow("LiDAR_Vision 2D Top View", top)

        self.handle_keyboard()
        self.print_status(lidar_points_base, persons, obstacle_state, lidar_clusters)
    ...
    def compute_lidar_obstacle_layer(self, lidar_points_base):
        ...
        if len(obs) > self.obstacle_draw_max_points:
            idx = np.random.choice(len(obs), self.obstacle_draw_max_points, replace=False)
            state["obstacle_points_draw"] = obs[idx].copy()
        else:
            state["obstacle_points_draw"] = obs.copy()
    ...
    # V3.4 LiDAR cluster candidate layer
    # ============================================================
    def compute_lidar_cluster_candidates(self, obstacle_state):
        """
    ...
    def draw_yolo_view(self, frame, persons):
        ...
        status = (
            f"LiDAR_Vision V3.4 | tracks={len(persons)} | control={control_count} | avg={avg_ms:.1f}ms | "
    ...
    def draw_top_view(self, persons, lidar_points_topview, obstacle_state=None, lidar_clusters=None):
    ...
    def draw_lidar_clusters_topview(self, canvas, clusters, origin_x, origin_y):
        for cluster in clusters:
            ...
            label = (
                ...
                f"{ctype} {cluster.get('distance', 0.0):.1f}m "
    ...
    def draw_topview_labels(self, canvas, origin_x, origin_y):
        ...
        cv2.putText(canvas,
                    "Gray: LiDAR | Orange dots: obstacle layer | C boxes: LiDAR clusters | Human colors: confidence",
    ...
    # Status
    # ============================================================
    def print_status(self, lidar_points, persons, obstacle_state=None, lidar_clusters=None):
        now = time.time()
...
def main(args=None):
    ...
    try:
        node = LiDARVisionTopViewV34Node()
        rclpy.spin(node)
