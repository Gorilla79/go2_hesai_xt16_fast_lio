...
class LiDARVisionTopViewV33Node(Node):
    """
    ...
    def __init__(self):
        super().__init__("lidar_vision_topview_v3_3_node")
        ...
        self.get_logger().info("LiDAR_Vision top-view V3.3 node started.")
        ...
        self.get_logger().info(
            f"V3.3 tracking enable={self.enable_tracking}, "
        ...
        self.get_logger().info(
            f"V3.3 duplicate filter enable={self.enable_duplicate_filter}, "
        ...
        self.get_logger().info(
            f"V3.3 false-positive filter enable={self.enable_false_positive_filter}, "
        ...
        self.get_logger().info(
            f"V3.3 LiDAR obstacle layer enable={self.enable_lidar_obstacle_layer}, "
    ...
    def main_loop(self):
        ...
        if self.draw_topview_window:
            top = self.draw_top_view(persons, lidar_points_topview, obstacle_state)
            cv2.imshow("LiDAR_Vision 2D Top View", top)

        self.handle_keyboard()
        self.print_status(lidar_points_base, persons, obstacle_state)
    ...
    # ============================================================
    # V3.3 false-positive suppression / adaptive confidence
    # ============================================================
    ...
    def draw_yolo_view(self, frame, persons):
        ...
        status = (
            f"LiDAR_Vision V3.3 | tracks={len(persons)} | control={control_count} | avg={avg_ms:.1f}ms | "
    ...
    def draw_top_view(self, persons, lidar_points_topview, obstacle_state=None):
    ...
    def draw_topview_labels(self, canvas, origin_x, origin_y):
        ...
        cv2.putText(canvas,
                    "Gray: LiDAR | Orange/Red dots: LiDAR obstacle layer | Green/Yellow/Orange/Red: human confidence",
    ...
    # Status
    # ============================================================
    def print_status(self, lidar_points, persons, obstacle_state=None):
        now = time.time()
...
def main(args=None):
    ...
    try:
        node = LiDARVisionTopViewV33Node()
        rclpy.spin(node)
