...
class LiDARVisionTopViewV31Node(Node):
    """
    ...
    def __init__(self):
        super().__init__("lidar_vision_topview_v3_1_node")
        ...
        self.get_logger().info("LiDAR_Vision top-view V3.1 node started.")
        ...
        self.get_logger().info(
            f"V3.1 tracking enable={self.enable_tracking}, "
        ...
        self.get_logger().info(
            f"V3.1 duplicate filter enable={self.enable_duplicate_filter}, "
    ...
    def get_control_human_count(self, persons):
        ...
        V3.1 policy:
            - ACTIVE tracks count as 1.
            - Very recent STALE tracks can optionally count as 1.
            - Older STALE tracks are visualization-only.
        """
        count = 0

        for p in persons:
            if not p.get("is_stale", False):
                count += 1
                continue

            if int(p.get("missed_frames", 999)) <= self.stale_control_max_missing:
                count += 1
    ...
    def draw_yolo_view(self, frame, persons):
        ...
        status = (
            f"LiDAR_Vision V3.1 | tracks={len(persons)} | control={control_count} | avg={avg_ms:.1f}ms | "
...
def main(args=None):
    ...
    try:
        node = LiDARVisionTopViewV31Node()
        rclpy.spin(node)
